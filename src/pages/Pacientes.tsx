...
const Pacientes = () => {
  ...
  return <div className="p-6 space-y-8 min-h-screen bg-indigo-950">
      <Header title="Pacientes" description="Gerencie seus pacientes convertidos de leads" />

      <PacientesStats pacientes={pacientes} />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <PacientesList pacientes={pacientes} searchTerm={searchTerm} setSearchTerm={setSearchTerm} selectedPatient={selectedPatient} setSelectedPatient={setSelectedPatient} onDeletePatient={handleDeletePatient} />

        <PatientProfile selectedPatient={selectedPatient} onOpenConsultaDialog={() => setConsultaDialogOpen(true)} />
      </div>

      {/* Dialog para registrar consulta */}
      {selectedPatient && <ConsultaRealizadaDialog open={consultaDialogOpen} onOpenChange={setConsultaDialogOpen} paciente={selectedPatient} />}

      {/* Dialog para excluir paciente */}
      <DeletePacienteDialog open={!!selectedPacienteForDelete} onOpenChange={open => {
      if (!open) {
        setSelectedPacienteForDelete(null);
      }
    }} paciente={selectedPacienteForDelete} onDeleteSuccess={handleDeleteSuccess} />
    </div>;
};
